...
try {
    ...
    $path = sprintf($upload_dir, sha1_file($_FILES['upfile']['tmp_name'][$i]), $ext);
    if (!move_uploaded_file( $_FILES['upfile']['tmp_name'][$i], $path )) {

        throw new RuntimeException('Failed to move uploaded file.');
    }
      
    array_push($uploadedfilesarray, str_replace("..","",$server_url.$path)); 
      }
    ...
    echo json_encode(array("message" => "File is uploaded successfully", "Path" => $uploadedfilesarray ));
